    ...
    namespace Kingsley {
        ...
        LinkControl::initialize(const std::string &port_name, const UnitAlgebra &link_bw_in,
            ...
            rtr_link = configureLink(port_name, std::string("1GHz"),
                                     new Event::Handler<LinkControl>(this,
                                                                     &LinkControl::handle_input));
        ...
        void LinkControl::init(unsigned int phase) {
            ...
            switch (init_state) {
                ...
                case 2: {
                    ...
                    credit_event *cr_ev = new credit_event(0, inbuf_size.getRoundedValue() /
                                                              flit_size);
                    ...
                    while ((ev = rtr_link->recvInitData()) != NULL) {
                        ...
                        switch (bev->getType()) {
                            case BaseNocEvent::CREDIT: {
                                ...
                                if (ce->vn <
                                    req_vns) {  // Ignore credit events for VNs I don't have
...
// otherwise.
        bool LinkControl::send(SimpleNetwork::Request *req, int vn) {
            ...
            if (ev->getTraceType() != SimpleNetwork::Request::NONE) {
                ...
                PRIu64
                " ns: Send on LinkControl in NIC: %s\n", ev->getTraceID(),
...
// the next event.
        SST::Interfaces::SimpleNetwork::Request *LinkControl::recv(int vn) {
            ...
            // into account.
            rtr_link->send(
                new credit_event(event->request->vn, in_ret_credits[event->request->vn]));
