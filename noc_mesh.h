...
class noc_mesh : public Component {
  ...
  public:
    ...
    SST_ELI_DOCUMENT_PARAMS(
        ...
        {"port_priority_equal",
         "Set to true to have all port have equal priority (usually endpoint ports have higher priority).", "false"},
        {"route_y_first", "Set to true to rout Y-dimension first.", "false"},
        {"use_dense_map", "Set to true to have a dense network id map instead of the sparse map normally used.",
         "false"},
        // {"network_inspectors", "Comma separated list of network inspectors to put on output ports.", ""},
    )

    SST_ELI_DOCUMENT_PORTS({"north", "North port", {}}, {"south", "South port", {}}, {"east", "East port", {}},
                           {"west", "West port", {}}, {"local%(local_ports)d", "Ports which connect to endpoints.", {}})
...
class noc_mesh_event : public BaseNocEvent {
  ...
  public:
    ...
    virtual noc_mesh_event *clone() override {
