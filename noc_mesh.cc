...
noc_mesh::noc_mesh(ComponentId_t cid, Params &params)
    ...
    auto *dummy_handler = new Event::Handler<noc_mesh, int>(this, &noc_mesh::handle_input_r2r, -1);
    ...
    for (int i = 0; i < local_ports; ++i) {
        ...
        port_name << "local";
        port_name << i;
        ports[local_port_start + i] = configureLink(
            port_name.str(),
            // new
            // Event::Handler<noc_mesh,int>(this,&noc_mesh::handle_input_ep2r,local_port_start+i));
            dummy_handler);

        // stats
        send_bit_count[local_port_start + i] =
            registerStatistic<uint64_t>("send_bit_count", port_name.str());
        output_port_stalls[local_port_start + i] =
            registerStatistic<uint64_t>("output_port_stalls", port_name.str());
        xbar_stalls[local_port_start + i] =
            registerStatistic<uint64_t>("xbar_stalls", port_name.str());
    }
...
void noc_mesh::route(noc_mesh_event *event) {
    if (route_y_first) {
        // Compute next port
...
void noc_mesh::handle_input_r2r(Event *ev, int port) {
    // TraceFunction trace(CALL_INFO);
    // Check type of event
    auto *base_ev = dynamic_cast<BaseNocEvent *>(ev);
    switch (base_ev->getType()) {
        case BaseNocEvent::CREDIT: {
            auto *credit_ret = dynamic_cast<credit_event *>(ev);
            port_credits[port] += credit_ret->credits;
            // output.output("(%d,%d): Got credit event for VN %d with %d
            // credits\n",my_x,my_y,credit_ret->vn,credit_ret->credits);
            delete ev;
            break;
        }
        case BaseNocEvent::INTERNAL: {
            auto *event = dynamic_cast<noc_mesh_event *>(ev);
...
auto noc_mesh::wrap_incoming_packet(NocPacket *packet) -> noc_mesh_event * {
    // Wrap the incoming NocPacket in a noc_mesh_event
    auto *event = new noc_mesh_event(packet);
    ...
    if (x == 0) {
        ...
        // output.output("****** Setting egress port to: %d -> %d, %d, %d,
    ...
    //     output.output("dest %lld (%d) routed to router (%d,%d) with egress
...
void noc_mesh::handle_input_ep2r(Event *ev, int port) {
    // TraceFunction trace(CALL_INFO);
    // Check type of event
    auto *base_ev = dynamic_cast<BaseNocEvent *>(ev);
    switch (base_ev->getType()) {
        ...
        case BaseNocEvent::PACKET: {
            auto *packet = dynamic_cast<NocPacket *>(ev);
...
auto noc_mesh::clock_handler(Cycle_t cycle) -> bool {
    last_time = cycle;
    // TraceFunction trace(CALL_INFO);
    ...
    for (auto &lru : lru_units) {
        for (unsigned int i = 0; i < lru.size(); i++) {
            ...
            if (!port_queues[lru_port].empty()) {
                ...
                // that port
                // output.output("(%d,%d): clock_handler(): port_credits[%d] =
                ...
                if (port_credits[port] >= event->encap_ev->getSizeInFlits()) {
                    ...
                    SST::Interfaces::SimpleNetwork::Request::TraceType ttype =
                        event->encap_ev->request->getTraceType();
                    ...
                    if (ttype == SimpleNetwork::Request::FULL) {
                        output.output("TRACE(%d): %" PRIu64
                                      " ns: Sent an event to router from router: (%d,%d)"
                                      " (%s) on VC %d from src %" PRIu64 " to dest %" PRIu64 ".\n",
                                      trace_id, getCurrentSimTimeNano(), my_x, my_y,
                                      getName().c_str(), vn, src, dest);
                    }
                    // Need to send credit event back to last router
                    auto *cr_ev = new credit_event(0, flits);
...
void noc_mesh::init(unsigned int /*phase*/) {
    ...
    switch (init_state) {
        ...
        case 1: {
            ...
            for (int i = 0; i < local_port_start + local_ports; ++i) {
                if (ports[i] == nullptr) {
                    ...
                    if (ev != nullptr) {
                        nie = dynamic_cast<NocInitEvent *>(ev);
                        if (nie->command == NocInitEvent::REPORT_ENDPOINT) {
                            ...
                            ports[i]->setFunctor(new Event::Handler<noc_mesh, int>(
                                this, &noc_mesh::handle_input_ep2r, i));
                        } else {
                            // Unexpected command
                        }
                    } else {
                        ...
                        ports[i]->setFunctor(new Event::Handler<noc_mesh, int>(
                            this, &noc_mesh::handle_input_r2r, i));
                    }
                }
            }

            // output.output("%s::init(%u) - edge_status =
            // 0x%x\n",getName().c_str(),phase,edge_status); output.output("%s::init(%u) -
            // endpoint_locations = 0x%x\n",getName().c_str(),phase,endpoint_locations);
        ...
        case 2:
            ...
            if (nie->command == NocInitEvent::SUM_ENDPOINTS) {
                nie->int_value += endpoint_start;
                ports[west_port]->sendInitData(nie);
                if ((edge_status & south_mask) != 0U) {  // southern row also computing x_size
                    ev = ports[east_port]->recvInitData();
                    nie = dynamic_cast<NocInitEvent *>(ev);
        ...
        case 3:
            ...
            if ((edge_status & south_mask) != 0U) {
                ...
                init_state = 5;
            } else if ((edge_status & north_mask) != 0U) {  // northwest corner
        ...
        case 6:
            ...
            if (nie->command == NocInitEvent::BROADCAST_TOTAL_ENDPOINTS) {
                ...
                if ((edge_status & north_mask) == 0U) {
                    ports[north_port]->sendInitData(nie->clone());
                }
                ports[east_port]->sendInitData(nie);
            }
        ...
        case 7:
            ...
            init_state = 8;
            if (!(((edge_status & north_mask) != 0U) && ((edge_status & east_mask) != 0U))) {
        ...
        case 8: {
            ...
            if ((endpoint_locations & north_mask) != 0U) {
                int x = my_x;
                int y = my_y + 1;
                ...
                int tmp = north_port;
                ep_ids.emplace_back(endpoint_id, tmp);
            }

            if ((endpoint_locations & south_mask) != 0U) {
                int x = my_x;
                int y = my_y - 1;
                int endpoint_id = ((y * x_size) + x) * local_ports;
                // This is odd, but a static const int does not allocate
            ...
            if ((endpoint_locations & east_mask) != 0U) {
                int x = my_x + 1;
                int y = my_y;
                int endpoint_id = ((y * x_size) + x) * local_ports;
                // This is odd, but a static const int does not allocate
            ...
            if ((endpoint_locations & west_mask) != 0U) {
                int x = my_x - 1;
                int y = my_y;
                int endpoint_id = ((y * x_size) + x) * local_ports;
                // This is odd, but a static const int does not allocate
            ...
            // Now for local ports
            for (int i = 0; i < local_ports; ++i) {
                if ((endpoint_locations & (1 << (i + local_port_start))) != 0U) {
                    int endpoint_id = (((my_y * x_size) + my_x) * local_ports) + i;
                    ep_ids.emplace_back(endpoint_id, local_port_start + i);
                }
            }
            ...
            if (use_dense_map) {
                SharedRegion *sr = Simulation::getSharedRegionManager()->getGlobalSharedRegion(
                    "noc_mesh_dense_map", 16 * total_endpoints * sizeof(int),
        ...
        case 9: {
            for (int i = 0; i < local_port_start + local_ports; ++i) {
                if (ports[i] != nullptr) {
                    auto *cr_ev = new credit_event(0, input_buf_size / flit_size);
        ...
        case 10: {
            ...
            for (int i = 0; i < local_port_start + local_ports; ++i) {
                if (ports[i] != nullptr) {
                    auto *cr_ev = dynamic_cast<credit_event *>(ports[i]->recvInitData());
        ...
        default:
            ...
            for (int i = 0; i < local_port_start + local_ports; ++i) {
                if (ports[i] != nullptr) {
                    bool endpoint = ((1 << i) & endpoint_locations) != 0U;
                    while (true) {  // Go until there are no more events
                        ...
                        if (endpoint) {
                            ...
                            nme = dynamic_cast<noc_mesh_event *>(ev);
                        }
                        ...
                        if (nme->egress_port == -1) {
                            ...
                            if (endpoint || (((1 << i) & east_mask) != 0)) {
                                // No need to send east if this is the
                                // eastern edge
                                if ((edge_status & west_mask) == 0U) {
                            ...
                            // south.
                            if (endpoint || (((1 << i) & west_mask) != 0) ||
                                (((1 << i) & east_mask) != 0) || (((1 << i) & south_mask) != 0)) {
                            ...
                            // north.
                            if (endpoint || (((1 << i) & west_mask) != 0) ||
                                (((1 << i) & east_mask) != 0) || (((1 << i) & north_mask) != 0)) {
                            ...
                            nme->encap_ev = nullptr;
                            delete nme;
                            for (int j = 0; j < local_port_start + local_ports; ++j) {
                                if (endpoint && (i == j)) {
                                    continue;  // No need to send back to src
                                }
                                if (((1 << j) & endpoint_locations) != 0U) {
                                    if (!sent) {
                            ...
                            route(nme);
                            if (((1 << nme->next_port) & endpoint_locations) != 0U) {
...
void noc_mesh::complete(unsigned int /*phase*/) {
    // Simply route messages that are sent by the endpoints
    for (int i = 0; i < local_port_start + local_ports; ++i) {
        if (ports[i] != nullptr) {
            bool endpoint = ((1 << i) & endpoint_locations) != 0U;
...
void noc_mesh::printStatus(Output &out) {
    out.output("Start Router %s:  id = (%d, %d)\n", getName().c_str(), my_x, my_y);

    std::vector<std::pair<std::string, int>> vec;
    int port = north_port;
    vec.emplace_back("North", port);
    port = south_port;
    vec.emplace_back("South", port);
    port = east_port;
    vec.emplace_back("East", port);
    port = west_port;
    vec.emplace_back("West", port);

    for (int i = 0; i < local_ports; ++i) {
        std::string str = "local_port" + std::to_string(i);
        vec.emplace_back(str, i + local_port_start);
    }
