...
noc_mesh::noc_mesh(ComponentId_t cid, Params &params) :
    ...
    Event::Handler<noc_mesh, int> *dummy_handler = new Event::Handler<noc_mesh, int>(this,
                                                                                     &noc_mesh::handle_input_r2r,
                                                                                     -1);
    ...
    for (int i = 0; i < local_ports; ++i) {
        ...
        // stats
        send_bit_count[local_port_start + i] = registerStatistic<uint64_t>("send_bit_count",
                                                                           port_name.str());
        output_port_stalls[local_port_start + i] = registerStatistic<uint64_t>("output_port_stalls",
                                                                               port_name.str());
        xbar_stalls[local_port_start + i] = registerStatistic<uint64_t>("xbar_stalls",
                                                                        port_name.str());
    }
                ...
                if (ports[i] == NULL) {
                    ...
                    if (ev != NULL) {
                        ...
                        if (nie->command == NocInitEvent::REPORT_ENDPOINT) {
                            ...
                            ports[i]->setFunctor(new Event::Handler<noc_mesh, int>(this,
                                                                                   &noc_mesh::handle_input_ep2r,
                                                                                   i));
                        } else {
                            // Unexpected command
                        }
                        ...
                        ports[i]->setFunctor(
                            new Event::Handler<noc_mesh, int>(this, &noc_mesh::handle_input_r2r,
                                                              i));
                    }
                }
            }
