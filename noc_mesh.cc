...
noc_mesh::noc_mesh(ComponentId_t cid, Params &params)
    : Component(cid), init_state(0), init_count(0), endpoint_start(0), total_endpoints(0), edge_status(0),
      endpoint_locations(0), use_dense_map(false), dense_map(nullptr),
...
void noc_mesh::handle_input_r2r(Event *ev, int port) {
    ...
    // Check type of event
    auto *base_ev = static_cast<BaseNocEvent *>(ev);
    switch (base_ev->getType()) {
    case BaseNocEvent::CREDIT: {
        auto *credit_ret = static_cast<credit_event *>(ev);
    ...
    case BaseNocEvent::INTERNAL: {
        auto *event = static_cast<noc_mesh_event *>(ev);
...
noc_mesh_event *noc_mesh::wrap_incoming_packet(NocPacket *packet) {
...
void noc_mesh::handle_input_ep2r(Event *ev, int port) {
    ...
    case BaseNocEvent::PACKET: {
        auto *packet = static_cast<NocPacket *>(ev);
...
bool noc_mesh::clock_handler(Cycle_t cycle) {
    last_time = cycle;
    // TraceFunction trace(CALL_INFO);
                ...
                if (port_credits[port] >= event->encap_ev->getSizeInFlits()) {
                    ...
                    if (ttype == SimpleNetwork::Request::FULL) {
                        output.output("TRACE(%d): %" PRIu64 " ns: Sent an event to router from router: (%d,%d)"
                                      ...
                                      trace_id, getCurrentSimTimeNano(), my_x, my_y, getName().c_str(), vn, src, dest);
                    }
...
void noc_mesh::init(unsigned int /*phase*/) {
    ...
    case 8: {
        ...
        if (use_dense_map) {
            SharedRegion *sr = Simulation::getSharedRegionManager()->getGlobalSharedRegion(
                "noc_mesh_dense_map", 16 * total_endpoints * sizeof(int), new SharedRegionMerger());
    ...
    case 10: {
        ...
        for (int i = 0; i < local_port_start + local_ports; ++i) {
            if (ports[i] != nullptr) {
                auto *cr_ev = static_cast<credit_event *>(ports[i]->recvInitData());
            ...
            if (ports[i] != nullptr) {
                ...
                while (true) { // Go until there are no more events
                    ...
                    if (nme->egress_port == -1) {
                        ...
                        // south.
                        if (endpoint || ((1 << i) & west_mask) || ((1 << i) & east_mask) || ((1 << i) & south_mask)) {
                        ...
                        // from an endpoint, or from the east, west or
                        // north.
                        if (endpoint || ((1 << i) & west_mask) || ((1 << i) & east_mask) || ((1 << i) & north_mask)) {
    ...
    for (auto &pinfo : vec) {
        ...
        if (ports[pinfo.second] != nullptr) {
            ...
            if (port_queues[pinfo.second].empty()) {
                ...
                out.output("      src = %lld, dest = %lld, next_port = %d, flits = %d\n", event->encap_ev->request->src,
                           event->encap_ev->request->dest, event->next_port, event->encap_ev->getSizeInFlits());
            }
        } else {
            out.output("    UNUSED\n");
        }
